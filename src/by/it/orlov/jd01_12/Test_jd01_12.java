...
public class Test_jd01_12 {
    ...
    public void testTaskB2() throws Exception {
        ...
        Method mA = findMethod(aclass, "action", ArrayList.class);
        mA.setAccessible(true);
        Method mL = findMethod(aclass, "action", LinkedList.class);
        ...
        for (int n = 1; n <= expectedRes.length; n++) {
            ...
            System.out.println("  action(ArrayList  arr)=" + "n" + expectedRes[n - 1]);
            System.out.flush();
            assertEquals("Метод с LinkedList работает неверно", mL.invoke(null, arrl), "n" + expectedRes[n - 1]);
            System.out.println("  action(LinkedList arr)=" + "n" + expectedRes[n - 1]);
            System.out.flush();
        }
